...
def get_norm(norm_type:str, num_channels:int, num_groups:int):
    if norm_type == "instance":
        # 如果affine==True，则对归一化后的batch进行仿射变换，
        # 即乘以模块内部的weight（初值是[1., 1., 1., 1.]）
        # 然后加上模块内部的bias（初值是[0., 0., 0., 0.]）
        # 这两个变量会在反向传播时得到更新。
...
class PositionalEmbedding(nn.Module):
    def __init__(self, dim:int, scale=1.0) -> None:
        super().__init__()
        assert dim % 2 == 0
        self.dim = dim
        self.scale = scale
